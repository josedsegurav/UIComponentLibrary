...
describe("Dropdown Component", () => {
  ...
  it("applies different sizes correctly", () => {
    const sizeTests = [
      { size: "large" as const, expectedPadding: "clamp(12px, 3vw, 16px) clamp(40px, 10vw, 48px) clamp(12px, 3vw, 16px) clamp(16px, 4vw, 20px)" },
      { size: "small" as const, expectedPadding: "clamp(6px, 1.5vw, 8px) clamp(32px, 8vw, 40px) clamp(6px, 1.5vw, 8px) clamp(12px, 3vw, 16px)" },

    ];

    sizeTests.forEach(({ size, expectedPadding }, index) => {
      render(<Dropdown {...defaultProps} $size={size} />);
      const dropdown = screen.getAllByRole("combobox")[index] as HTMLSelectElement;
